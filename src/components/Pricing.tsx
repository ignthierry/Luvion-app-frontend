...
function PricingCardItem({
  ...
  return (
    <motion.div
      ...
      className={`relative p-8 rounded-[2rem] transition-all duration-500 flex flex-col justify-between overflow-hidden glass-panel hover:-translate-y-2 hover:scale-[1.03] hover:shadow-[0_30px_60px_-15px_rgba(0,88,188,0.25)] hover:z-20 ${
        displayHighlight
          ? 'border-primary ring-2 ring-primary/20 scale-[1.03] shadow-[0_24px_48px_-12px_rgba(0,88,188,0.1)] z-10'
          : 'opacity-90 hover:opacity-100'
      }`}
    >
      {/* Dynamic Cursor Spotlight Glass Reflection */}
      <div
        className="absolute inset-0 rounded-[2rem] pointer-events-none z-30 transition-opacity duration-700"
        style={{
          opacity: isHovered ? 1 : 0,
          background: `radial-gradient(450px circle at ${glarePosStyle.x}% ${glarePosStyle.y}%, rgba(255, 255, 255, 0.4), rgba(255, 255, 255, 0.05) 40%, transparent 75%)`,
        }}
      />

      {/* Slow, Organic Glass Sheen Beam Sweep */}
      <div className="absolute inset-0 rounded-[2rem] overflow-hidden pointer-events-none z-20">
        <motion.div
          ...
          className="w-[75%] h-[350%] -top-[125%] -rotate-[30deg] bg-gradient-to-r from-transparent via-white/40 dark:via-white/20 to-transparent blur-[2px]"
        />
      </div>
